...
if(isset($_POST['payment_method_nonce'])){
	...
	if($result->success == true){
			...
			if($result_booking !== false && $db->last_row_count() > 0){
                ...
                if($payment_amount){
					...
					if($result_booking->execute() !== false){
					    ...
					    $data_a = array();
                        $data_a['id'] = null;
                        $data_a['id_booking'] = $id_booking;
                        $data_a['date'] = time();
                        $data_a['descr'] = 'Payment has been authorised by braintree';
                        $data_a['trans'] = $txn_id;
                        $data_a['method'] = 'card';
                        $data_a['amount'] = $payment_amount;
						...
						$mail = getMail($db, 'BOOKING_CONFIRMATION', array(
							...
							'{payment_notice}' => 'Payment has been authorised by braintree'
						));
						...
						$payment_notice = '<h2>Your booking confirmation id  <b>'.$_SESSION['booking_id'].'</b></h2>><p>';
						$payment_notice .= str_replace('{amount}', '<b>'.formatPrice($payed_amount*CURRENCY_RATE).' '.$texts['INCL_VAT'].'</b>', $texts['PAYMENT_CHECK_NOTICE']).'</p>';
						$_SESSION['msg']=$payment_notice;
		...
		echo $result->message;// Payment hasn't been authorised
