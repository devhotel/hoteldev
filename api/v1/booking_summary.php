...
if($rowAccotax !== false && $db->last_row_count() > 0){ //Check if Accomodation Exist in Tax Table
	...
	$response=array('status'=>array('error_code'=>0,'message'=>'Success'), 'response'=>array('cart_amount'=>array('subTotal'=>number_format((float)$sum, 2, '.', ''),'taxName' => 'GST', 'taxValue' => '0', 'taxTotal'=>number_format((float)$taxAmount, 2, '.', ''), 'cartTotal'=>number_format((float)$totalAmount, 2, '.', ''))));
	displayOutput($response);
}