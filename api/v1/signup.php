...
if ($result_exists !== false && $db->last_row_count() > 0) {
	...
	if ($result_user->execute() !== false) {
		...
		if ($result_exists !== false && $db->last_row_count() > 0) {
			...
			$from_name = 'HMS';
			if (sendMail($email, 'Traveller', 'Welcome to HMS', $mailContent, '', '', $from_email, $from_name) !== false) {
				//Add Device Token
				$device = array();
				$device['id'] = null;
				$device['user_id'] = $nuid;
				...
				$body = 'Welcome to HMS. Your registration is successful.';
				...
				$noti['notification_desc'] = 'Welcome to HMS. Your registration is successful.';
