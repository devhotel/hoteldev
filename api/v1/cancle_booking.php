...
if ($result_exists !== false && $db->last_row_count() > 0) {
	...
	$bookingDates = $fromDAte = $db->query('SELECT total, FROM_UNIXTIME(from_date,"%Y-%m-%d") as checkin_date, FROM_UNIXTIME(to_date,"%Y-%m-%d") as checkout_date FROM pm_booking WHERE id = "' . $bookingId . '"')->fetch(PDO::FETCH_ASSOC);
	...
	if ($sqlCancle) {
		...
		$content1 = '<tr><td style="padding: 20px 20px 0 20px;background: #fff;"><p style="margin: 20px 0 20px;">Your booking ' . $bookingId . ' at Gupta Hotels for check-in date: ' . gmstrftime(DATE_FORMAT, $booking['from_date']) . ' has been cancelled.</p>';
		...
		$content = '<tr><td style="padding: 20px 20px 0 20px;background: #fff;"><p style="margin: 20px 0 20px;">Your booking ' . $bookingId . ' at Gupta Hotels for check-in date: ' . gmstrftime(DATE_FORMAT, $booking['from_date']) . ' has been cancelled.</p>';
