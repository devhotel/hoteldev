...
require_once(SYSBASE.'ghstats/includes/fn_module.php');

if(in_array('no_access', $permissions) || empty($permissions)){
    header('Location: '.DOCBASE.'ghstats/index.php');
    exit();
}

require_once(SYSBASE.'ghstats/includes/fn_actions.php');
...
if(isset($_GET['view'])){
    $view = $_GET['view'];
    if($view == 'list'){
        if(is_file('list.php')) require_once('list.php'); else require_once(SYSBASE.'ghstats/modules/default/list.php');
    }
    if($view == 'form'){
        if(is_file('form.php')) require_once('form.php'); else require_once(SYSBASE.'ghstats/modules/default/form.php');
    }
}else{
    header('Location: '.DOCBASE.'ghstats');
    exit();
}
