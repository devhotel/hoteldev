...
function add_item($db, $table, $result_insert, $id_lang)
{
    ...
    if($result_insert->execute() !== false){
        ...
        if(is_numeric($id) && $id > 0)
            $_SESSION['msg_success'][] = 'The '.'The'.MODULE.' '.$texts['ADD_SUCCESS'];
        else
            $_SESSION['msg_error'][] = 'The '.MODULE.' '.$texts['UPDATE_ERROR'];
    }else
        $_SESSION['msg_error'][] = 'The '.MODULE.' '.$texts['UPDATE_ERROR'];
...
function edit_item($db, $table, $result_update, $id, $id_lang)
{
    ...
    if($result_update->execute() !== false)
        $_SESSION['msg_success'][] = 'The '.MODULE.' '.$texts['UPDATE_SUCCESS'];
    else
        $_SESSION['msg_error'][] = 'The '.MODULE.' '.$texts['UPDATE_ERROR'];
...
function check($db, $table, $id, $value, $redirection = true)
{
    ...
    if($db->query('UPDATE '.$table.' SET checked = '.$value.' WHERE id = '.$id) !== false){
        if($redirection){
            if($value == 1) $msg = 'The '.MODULE.' '.$texts['ELMT_ENABLED'];
            if($value == 2) $msg = 'The '.MODULE.' '.$texts['ELMT_DISABLED'];
            if($value == 3) $msg = 'The '.MODULE.' '.$texts['ELMT_ARCHIVED'];
...
function delete_item($db, $id, $redirection = true)
{
    ...
    if($db->query('DELETE FROM pm_'.MODULE.' WHERE id = '.$id) !== false)
        if($redirection) $_SESSION['msg_success'][] = 'The '.MODULE.' '.$texts['DELETE_SUCCESS'];
    else
        if($redirection) $_SESSION['msg_error'][] = 'The '.MODULE.' '.$texts['UPDATE_ERROR'];
...
{
    global $texts;
    $modules_list = getModules('ghstats/modules');
