...
<script>
    ...
    $(function(){
        ...
        $('.new_entry').on('click', function(){
            ...
            var cloneid='dp'+Date.now();
          
            $('.typeahead', last_row).typeahead('destroy');
            row = last_row.clone();
            //row.find("input").attr("id",cloneid);
            ...
            row.appendTo(table);
            /* 
              //.find('input.datepicker').removeClass('hasDatepicker')
              row.find(':input.datepicker').each(function() {
                     $(this).datepicker();
                })*/
    ...
    <script>
        ...
        $(function() {
            ...
            if(empty($_SESSION['msg_error'])){ ?>
                $.ajax({
                    type: 'POST',
                    url: '<?php echo DOCBASE; ?>ghstats/includes/clear_tmp.php',
