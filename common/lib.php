...
function sendMail($recipient_email, $recipient_name, $subject, $content, $reply_email = '', $reply_name = '', $from_email = '', $from_name = '', $attachements = array(), $emails_copy = '')
{
    ...
    $body = '<html><head><meta charset="utf-8"></head><body><div style="width: 700px;margin: 0 auto;"><table cellspacing="0" cellpadding="0" style="width: 100%;border:10px solid #fafafa;"><tbody><tr><td style="text-align: center;padding: 20px 20px 0 20px;background: #fff;"><a href="' . DOCBASE . '" target="_blank"><img src="cid:header-mail" alt="" /></a>
    ...
    try {
        ...
        $mail->AddEmbeddedImage(SYSBASE . 'templates/' . TEMPLATE . '/images/logo.png', 'header-mail', 'header-mail.png');
        $mail->MsgHTML($body);
 ...
 *
 * @return float decimal format return $currency.number_format($price, 2, '.', ',');
 */
function formatPrice($price, $currency = CURRENCY_SIGN)
{
    ...
    if (function_exists('money_format')) {
        $price = round($price);
        //return preg_replace('/([,\.](\s)?('.CURRENCY_SIGN.')?)$/', '$2$3', money_format($format, $price));
 ...
 * @return string
 */
function getFromTemplateNew($path, $docbase = true)
{
...
function walletUpdate($id_booking, $id_user, $amount, $type, $purpose)
{
    global $db;
    ...
    $wallet_insert = "INSERT INTO pm_wallet_history(id_booking, user_id, amount, type, purpose, c_date) VALUES(:id_booking, :user_id, :amount, :type, :purpose, :c_date)";
    $stmt2 = $db->prepare($wallet_insert);
    $stmt2 = $stmt2->execute($wallet_history);
    $result_wallet = $db->query('SELECT * FROM pm_wallet WHERE user_id = ' . $id_user)->fetch();
    $credit_amount = $db->query('SELECT SUM(`amount`) as credit_amount FROM pm_wallet_history WHERE type="credit" AND user_id = ' . $id_user)->fetch();
    $debit_amount = $db->query('SELECT SUM(`amount`) as debit_amount FROM pm_wallet_history WHERE type="debit" AND user_id = ' . $id_user)->fetch();
    $tot_wallet_amount = $credit_amount['credit_amount'] - $debit_amount['debit_amount'];
    if (empty($result_wallet)) {
        ...
        $wallet__datainsert = "INSERT INTO pm_wallet(user_id, amount, added_date) VALUES(:user_id, :amount, :added_date)";
        $stmt3 = $db->prepare($wallet__datainsert);
        $stmt3 = $stmt3->execute($wallet_data);
    } else {
        $wallet_data = [
            'user_id' => $id_user,
            'amount' => $tot_wallet_amount
        ];
        $wallet__datainsert = "UPDATE pm_wallet SET amount=:amount WHERE user_id=:user_id";
        $stmt3 = $db->prepare($wallet__datainsert);
        $stmt3->execute($wallet_data);
    }
}
...
function getRandonOTP($digits = 4)
{
    $digits = 4;
...
// bhashsms
/*function httpPost($mobile,$message){
    $params = array(
               "user" => "zapper123",
               "pass" => "Met@159",
               "sender" => "GUPHTL",
...
// Hitech sms
function httpPost($contacts, $message)
{
...
function add_activity_log($id_user, $id_item, $module, $action, $purpose)
{
    global $db;
    $data = array();
    $data['id'] = 0;
    ...
    $data['add_date'] = time();
    $data['checked'] = 1;
    $activity_insert = db_prepareInsert($db, 'pm_activity_log', $data);
    if ($activity_insert->execute() !== false) {
        return $db->lastInsertId();
    }
}

//API Header Return JSON
...
function get_hotel_rating($ratings)
{
    if (!empty($ratings)) {
        $rt = array();
        ...
        foreach ($ratings as $key => $value) {
            if ($value['params'] != "") {
                ...
                if (!empty($param)) {
                    $avrg = array_sum($param) / count($param);
                    //$maxrate = $maxrate + $avrg;
        ...
        $avarage_rate = $maxrate / $total_rating;
        $html = '<div class="review_rating_box">';
        $ratinghtml = '';
        if (!empty($rt)) {
            $avr = array();
            foreach ($rt as $kk => $va) {
                ...
                $kk = strtolower(str_replace(' ', '_', $kk));
                if (!in_array($kk, $avr)) {
                    $avr[] = $kk;
                    $ratinghtml .= '<div class="hotels-hotel-review-with-Review">';
                    $ratinghtml .= '<input id="rating-321" name="input-1" value="' . $average . '" class="rating-321 rating-loading">';
                    $ratinghtml .= '<div class="hotels-hotel-with-Reviews_Label">' . ucwords(str_replace('_', ' ', $kk)) . '</div>';
                    $ratinghtml .= '</div>';
                }
            }
            //var_dump($avr);
        }
        $ravt = number_format($avarage_rate, 1);
        $html .= '<div class="rating_top">
                  <div class="hotels-hotel-review- with_Reviews_rating">
                     <span class="hotels-hotel-reviewt-with-Reviews_overallRating">' . $ravt . '</span>
                      <a href="#REVIEWS" class="hotels-hotel-review-about-with-photos-Reviews">
                        <span class="overallRating"><input id="ratingf" name="input-1" value="' . $ravt . '" class="rating-321 rating-loading" data-show-clear="false" data-show-caption="false" ></span>
                        <span class="hotels-hotel- seeAllReviews">' . $total_rating . ' reviews</span>
                      </a>
                </div>
               </div>';
        $html .= $ratinghtml;
...
function get_user_fcm_token($user_id)
{
    $tokenSql = $db->query('SELECT * FROM pm_device WHERE user_id  = "' . $user_id . '" AND device_type = 1');
    $rowToken = $tokenSql->fetch();
    if (!empty($rowToken)) {
        return $rowToken['fcm_token'];
    }
}

// Send push notification by device token
function push_notify_send($fcm_token, $deviece_type, $title, $message)
{
    ...
    if ($fcm_token != "") {
        $sound = 'default';
        if ($deviece_type == 1) { // For Android
            $notification->setTitle($title);
            $notification->setBody($message);
            $notification->setSound($sound);

            $firebase_token = $fcm_token;
            //$firebase_api = 'AAAA67yrYAk:APA91bFItT8oYU17ArhzvJSX5PDAlp_rINmGtsMYpj_t1mmviWaapWN3-gI7EXOq98I-Yazm-GYQdIosJhMG6CeWHi3k9ZIianFhjKvTzP7horhPyppBi25zNFS3Jk6TkIZ5KA220qTa';
            $firebase_api = 'AAAAygBrcow:APA91bGFTRfRL_rA3hoSN7Nha0_6KHtuZrztwJ1C_E10bWIxZjsSBe3UApusAxQmy54mhhlIStp5ZftXV_uQGaBxeLnXeJQYNj526vnFqUh4wWcbCAlfuT_PNw4KrFwrpateMfkn4SyN';
