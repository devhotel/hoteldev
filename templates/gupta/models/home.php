        ...
        <div class="carousel-inner" role="listbox">
            <?php for ($j = 0; $j < count($testimonial); $j++) { ?>
                <div class="item <?php if ($j == 0) {
                    ...
                    <h1>Gupta hotels , Luxury Hotels </h1>
                    <?= $testimonial[$j]['name'] ?>
...
<section id="content" class="pt20 pb30">
                                ...
                                if (is_file($realpath)) {
                                    ...
                                    <figure class="more-link">
                                        ...
                                        <div class="more-content">
                                            <!--  <h3 itemprop="name"><?php //echo $hotel_title; 
                                                                        ?></h3> -->
                                            <?php
                                            if ($min_price > 0) { ?>
    ...
    /*
    $v_slide_id = 0;
        $v_result_slide_file = $db->prepare('SELECT * FROM pm_vendors_file WHERE id_item = :v_slide_id AND checked = 1 AND lang = '.DEFAULT_LANG.' AND type = \'image\' AND file != \'\' ORDER BY rank LIMIT 1');
        $v_result_slide_file->bindParam('v_slide_id', $v_slide_id);

        $v_result_slide = $db->query('SELECT * FROM pm_vendors WHERE id_page = '.$page_id.' AND checked = 1 AND lang = '.LANG_ID.' ORDER BY rank', PDO::FETCH_ASSOC);
        if($v_result_slide !== false){
    ?>
    <div class="container">
   <section class="customer-logos slider">
       <?php 
        foreach($v_result_slide as $i => $row){
                    $v_slide_id = $row['id'];
                    $slide_legend = $row['legend'];
                    $url_v = $row['url'];
                    $id_page = $row['id_page'];
                    
                    $v_result_slide_file->execute();
                    
                    if($v_result_slide_file !== false && $db->last_row_count() == 1){
                        $row = $v_result_slide_file->fetch();
                        $file_id = $row['id'];
                        $filename = $row['file'];
                        $label = $row['label'];
                        
                        $realpath = SYSBASE.'medias/vendors/big/'.$file_id.'/'.$filename;
                        $thumbpath = DOCBASE.'medias/vendors/small/'.$file_id.'/'.$filename;
                        $zoompath = DOCBASE.'medias/vendors/big/'.$file_id.'/'.$filename;
                        ?>
      <div class="slide"><a href="<?php echo $url_v; ?>" target="_blank"><img src="<?php echo $zoompath; ?>"></a></div>
      <?php } }  ?>
   </section>
   <script>
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <h2>COVID-19 Safety Measures at Gupta Hotels</h2>
                <p>Each and every room gets sanitized after guests check out</p>
                <ul>
