...
if (isset($_POST) && isset($_POST['key'])) {
    ...
    if ($status == 'success') {
        ...
        $_SESSION['book']['down_payment'] = $postdata['amount'];
        $downPayment = $postdata['amount'];
    } else {
        //tampered or failed
...
if ($handle) {
    ...
    if (!isset($_SESSION['book']['id']) || is_null($_SESSION['book']['id'])) {
        ...
        if ($payment_type != 'arrival') $data['down_payment'] = $downPayment;
        $data['add_date'] = time();
        $data['edit_date'] = null;
        ...
        $data['status'] = 4;
        $data['coupon_used'] = (isset($_SESSION['book']['coupon']) ? $_SESSION['book']['coupon'] : '');
    ...
    if (isset($_SESSION['book']['id']) && $_SESSION['book']['id'] > 0) {
        ...
        if ($payment_type == 'pre-paid') {
            $data['status'] = 4;
            $data['trans'] = $payuMoneyId;
            $data['payment_date'] = time();
            ...
            $data_a['date'] = time();
            $data_a['descr'] = 'Payment Successfull through Online';
            $data_a['trans'] = 'online';
            $data_a['method'] = 'online';
    ...
    if (isset($_SESSION['book']['id']) && $_SESSION['book']['id'] > 0) {
        if ($payment_type == 'wallet' || $payment_type == 'arrival' || $payment_type == 'pre-paid') {
            $room_content = '';
            ...
            $payment_notice = '';
            if ($payment_type == 'pre-paid') $payment_notice .= str_replace('{amount}', '<b>' . formatPrice($downPayment * CURRENCY_RATE) . ' ' . $texts['INCL_VAT'] . '</b>', $texts['PAYMENT_CHECK_NOTICE']);
            ...
            $adminMessage =
                "Booking Received -
                \r\nHOTEL NAME - " . $_SESSION['book']['hotel'] . "
                \r\nBOOKING ID - " . $_SESSION['book']['id'] . "
                \r\nGUEST NAME - " . $_SESSION['book']['firstname'] . " " . $_SESSION['book']['lastname'] . "
                \r\nCHECK IN DATE - " . gmstrftime(DATE_FORMAT, $_SESSION['book']['from_date']) . "
                \r\nCHECK OUT DATE - " . gmstrftime(DATE_FORMAT, $_SESSION['book']['to_date']) . "
                \r\nROOM - " . $bookedRoom . "
                \r\nAMOUNT - " . formatPrice($gtotal * CURRENCY_RATE) . "
                \r\nPAYMENT AT - " . $payment_type . "
                \r\nBOOKING SOURCE - WEBSITE
                \r\nGUEST MOBILE NO - " . $_SESSION['book']['mobile'];
...
<!-- PAYUMONEY BOLT Sandbox/test //-->
<!--<script id="bolt" src="https://sboxcheckout-static.citruspay.com/bolt/run/bolt.min.js" bolt--->
<!--color="e34524" bolt-logo="<?= getFromTemplate('images/logo.png') ?>"></script>-->
<!-- <script id="bolt" src="https://sboxcheckout-static.citruspay.com/bolt/run/bolt.min.js" bolt-color="e34524" bolt-logo="https://templates/gupta/images/logo.png"></script> -->
<!-- PAYUMONEY BOLT Production/Live //-->
<script id="bolt" src="https://checkout-static.citruspay.com/bolt/run/bolt.min.js" bolt-color="e34524" bolt-logo="https://guptahotels.com/templates/gupta/images/logo.png"></script>
<section id="page">
                                ...
                                <section class="paymenttab_wrapper payment-bg-paypg">
                                    ...
                                    <div class="container">
                        				...
                        				<li role="presentation"><a href="#walletupi" aria-controls="messages" role="tab" data-toggle="tab">Wallet/ UPI</a></li>-->
                                                ...
                                                <li role="presentation"><a href="#creditcard" aria-controls="home" role="tab" data-toggle="tab">Credit/Debit Card</a></li>
                                            </ul>
                                            <?php
                                                    ...
                                                    <div class="form-container">
                                                        <div class="add-payment-method-form-view">
                                                            <div class="text-center">
                                                                <?php //echo $texts['PAYMENT_CARDS_NOTICE']; 
                                                                ?><br>
                                                                <!--<img src="<?php echo getFromTemplate('images/payumoney.png'); ?>" alt="Pay-U-Money" class="img-responsive mt10 mb30">-->
                                                                <h2>Amount to Pay : <?= number_format(str_replace(',', '.', $payed_amount), 2, '.', '') ?></h2>
                                                                <form action="#" id="payment_form">
                                                                    <input type="hidden" id="udf5" name="udf5" value="BOLT_KIT_PHP7" />
                                                                    <input type="hidden" id="surl" name="surl" value="<?php echo getCallbackUrl(); ?>" />
                                                                    <input type="hidden" id="key" name="key" placeholder="Merchant Key" value="<?= PAYU_KEY ?>" />
                                                                    <input type="hidden" id="salt" name="salt" placeholder="Merchant Salt" value="<?= PAYU_SALT ?>" />
                                                                    <input type="hidden" id="txnid" name="txnid" placeholder="Transaction ID" value="<?= "Txn" . rand(10000, 99999999) ?>" />
                                                                    <input type="hidden" id="amount" name="amount" placeholder="Amount" value="<?= number_format(str_replace(',', '.', $payed_amount), 2, '.', '') ?>" />
                                                                    <input type="hidden" id="pinfo" name="pinfo" placeholder="Product Info" value="Hotel Booking <?= $_SESSION['book']['hotel_id'] ?>" />
                                                                    <input type="hidden" id="fname" name="fname" placeholder="First Name" value="<?= $_SESSION['book']['firstname'] . ' ' . $_SESSION['book']['lastname'] ?>" />
                                                                    <input type="hidden" id="email" name="email" placeholder="Email ID" value="<?= $_SESSION['book']['email'] ?>" />
                                                                    <input type="hidden" id="mobile" name="mobile" placeholder="Mobile/Cell Number" value="<?= $_SESSION['book']['phone'] ?>" />
                                                                    <input type="hidden" id="hash" name="hash" placeholder="Hash" value="" />
                                                                    <div><input type="submit" class="btn btn-primary btn-lg" value="Proceed To Pay" onclick="launchBOLT(); return false;" /></div>
                                                                </form>
...
<script type="text/javascript">
    ...
    function payUHashGenerator() {
        $.ajax({
            url: '<?php echo getFromTemplate('common/get_payumoney_hash.php'); ?>',
            type: 'post',
            data: JSON.stringify({
                key: $('#key').val(),
                salt: $('#salt').val(),
