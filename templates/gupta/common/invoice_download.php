...
if ($id_booking > 0) {
    ...
    if ($result_booking !== false && $db->last_row_count() > 0) {
        ...
        if ($result_cancel !== false && $db->last_row_count() > 0) {
            $html .= '<tr>';
            $html .= '<td class="text-right" width="100%" style="padding:15px;"><h2>Cancel Information</h2>
                                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">';
            foreach ($result_cancel as $cancel) {
                $html .= '<div class="booking_cancel">
                                  <p style="text-transform: capitalize; ">Cancel  Mode : <strong>' . $cancel['cancel_type'] . '</strong></p>
                                  <p>Cancel Reason : <strong>' . $cancel['reason'] . '</strong></p>
                                  <p>Cancellation   Charge :<strong>' . formatPrice($cancel['refund_charge'] * CURRENCY_RATE) . '</strong></p>
                                  <p>Refund amount : <strong>' . formatPrice($cancel['refund_amount'] * CURRENCY_RATE) . '</strong></p>
                                  <p>Cancelled : <strong>' . $cancel['cancel_element'] . '</strong></p>
                                  <p>Cancelled Date : <strong>' . date('D, jS F Y', strtotime($cancel['added_date'])) . '</strong></p>
                                 </div>';
            }
            $html .= '</div></td></tr>';
        }
        ...
        if ($result_payment !== false && $db->last_row_count() > 0) {
            $html .= '
                            <tr>
                                <th class="text-right" style="padding:15px;text-align: left;text-transform: uppercase;">Paid Amount (' . $texts['INCL_TAX'] . ')</th>
                                <td class="text-right" width="15%" style="padding:15px;"><b>' . formatPrice($row['paid'] * CURRENCY_RATE) . '</b></td>
                            </tr>';
        } else {
            $html .= '
                            <tr>
                                <th class="text-right" style="padding:15px;text-align: left;text-transform: uppercase;">Due Amount (' . $texts['INCL_TAX'] . ')</th>
