...
if (isset($_POST['booking_id']) && isset($_SESSION['user']['id']) && isset($_POST['reason'])) {
	...
	$cancel_elements = '';
	$bookingDates = $db->query('SELECT id_user, total, FROM_UNIXTIME(from_date,"%Y-%m-%d") as checkin_date, FROM_UNIXTIME(to_date,"%Y-%m-%d") as checkout_date FROM pm_booking WHERE id = "' . $id_booking . '"')->fetch(PDO::FETCH_ASSOC);
	$refundCharge = $_POST['refundCharge'];
	...
	if ($days == $result_booking['nights'] && $rcount == count($_POST['no_room'])) {
		...
		$cancel_type = 'full';
		$stat = 2;
		$booking_amount = $refundCharge - $refundAmount;
		$new_from_date = $result_booking['from_date'];
		$new_to_date = $result_booking['to_date'];
		$cancel_elements = 'Full (Date ( ' . $datesBetween . ' ) + Room ( ' . $room_detail['title'] . ' ))';
		$logPurpose = 'Booking Id ' . $id_booking . ' Cancelled ' . $cancel_elements . ' on ' . date('d-m-Y') . ' - Online mode';
	...
	add_activity_log($bookingDates['id_user'], $id_booking, 'booking', 'cancel', $logPurpose);
