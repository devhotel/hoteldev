...
require_once("../../../../common/define.php");
    ...
    $sign_user = htmlentities($_POST['email'], ENT_COMPAT, 'UTF-8');
    $uname = htmlentities($_POST['name'], ENT_COMPAT, 'UTF-8');
    $name = explode(' ', $uname);
    $fname= reset($name);
    $lname=end($name);
    if($sign_user != ''){
        ...
        if($result_user->execute() !== false){
                ...
                $from_name = 'HMS';
                if(sendMail($email, $uname, 'Welcome to '.SITE_TITLE , $mailContent,'','',$from_email,$from_name) !== false){
                    $response['success'] = 'success';
