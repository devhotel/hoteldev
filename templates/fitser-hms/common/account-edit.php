    ...
    if(count($response['notices']) == 0){
        ...
        if($result_user->execute() !== false){
            ...
            if($action == 'edit'){
                ...
                $mailContent = '
                <p><br>You created a new account.<br>
                  Please click on the link bellow to validate your account:<br>After first successful click message should be "Link has been expired."<br>
                  <a style="border: 2px solid #ff9800;background-color: white;color: black;padding: 10px 20px;font-size: 14px;cursor: pointer;" href="'.getUrl().'?token='.$token.'&id='.$db->lastInsertId().'&redirect='.urlencode($signup_redirect).'">Active your account</a></p>';
                ...
                if(isset($_POST['booksign']) && $_POST['booksign']==1){
