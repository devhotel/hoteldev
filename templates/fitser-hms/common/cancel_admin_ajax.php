...
if (isset($_POST['booking_id']) && isset($_SESSION['admin']['id']) && isset($_POST['reason'])) {
	...
	$db->query("INSERT INTO `pm_booking_cancel`(`id_booking`, `id_room`, `cancel_type`, `reason`, `booking_amount`, `refund_charge`, `refund_amount`) 
	VALUES ('" . $id_booking . "', '" . $id_room . "', 'full', '" . $reason . "', '" . $amount . "', '0.00', '" . $amount . "')");
	...
	// $db->prepare($sql_insert)->execute();
	//$stmt1->execute();
	//db_prepareInsert($db, 'pm_booking_cancel', $data1);
	//echo 	$stmt1 = $db->prepare($sql_insert);
	//echo 	$stmt1 = $stmt1->execute($data1);
	if (isset($_POST['booking_id'])) {
		...
		$purpose = 'Booking  ' . $id_booking . ' Cancelled';
		// Add activity log
		add_activity_log($_SESSION['admin']['id'], $id_booking, 'booking', 'edit', $purpose);
		...
		$content1 = '<tr><td style="padding: 20px 20px 0 20px;background: #fff;"><p style="margin: 20px 0 20px;">Your booking ' . $result_booking['id'] . ' at HMS
    			<p style="margin: 0 0 20px;">We hope to host you in the future.</p>
    			<p style="margin: 0 0 20px;">Cheers <br>Team HMS </p></td></tr>';

		$content = '<tr><td style="padding: 20px 20px 0 20px;background: #fff;"><p style="margin: 20px 0 20px;">Your booking ' . $result_booking['id'] . ' at HMS
    			<p style="margin: 0 0 20px;">We hope to host you in the future.</p>
    			<p style="margin: 0 0 20px;">Cheers <br>Team HMS </p></td></tr>';
