                    ...
                    <div class="mb5 mt5 bg-success">
                        ...
                        <div class="col-md-3 col-lg-2 pt5 pb5">
                            <div class="input-group input-group-sm">
                                ...
                                <select name="num_adults['.$room_id.']['.$i.']" class="form-control adult_select sendAjaxForm selectpicker" data-extratarget="#booking-amount_'.$hotel_id.'" data-action="'.getFromTemplate('common/change_num_people_book.php').'?index='.$i.'&id_room='.$room_id.'&id_hotel='.$hotel_id.'" data-target="#room-result-'.$room_id.'-'.$i.'">
                                    ';
                                    for($j = 1; $j <= $max_adults; $j++){
                                       $selected= ((isset($_SESSION['ab']['adlts'][$i])&& $_SESSION['ab']['adlts'][$i]==$j)?' selected="selected"':'');
                        ...
                        <div class="col-md-3 col-lg-2 pt5 pb5">
                            <div class="input-group input-group-sm">
                                ...
                                <select name="num_children['.$room_id.']['.$i.']" class="form-control kids_select sendAjaxForm selectpicker" data-extratarget="#booking-amount_'.$hotel_id.'" data-action="'.getFromTemplate('common/change_num_people_book.php').'?index='.$i.'&id_room='.$room_id.'&id_hotel='.$hotel_id.'" data-target="#room-result-'.$room_id.'-'.$i.'">
                                    <option value="0">0</option>';
                                    for($j = 1; $j <= $max_children; $j++){
                                        $selected= ((isset($_SESSION['ab']['kids'][$i])&& $_SESSION['ab']['kids'][$i]==$j)?' selected="selected"':'');
                                        $response['html'] .= '<option value="'.$j.'" '.$selected.'>'.$j.'</option>';
                                    }
                                    $response['html'] .= '
                                </select>
