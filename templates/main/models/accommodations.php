    ...
    <div id="content" class="pt30 pb20">
        <div class="container">
            ...
            <div class="row">
                ...
                $lz_pages = 0;
                $num_records = 0;
                $result = $db->query('SELECT count(*) FROM pm_accommodation WHERE checked = 1 AND lang = '.LANG_ID);
                if($result !== false){
                    $num_records = $result->fetchColumn(0);
                    $lz_pages = ceil($num_records/$lz_limit);
                }
                 if($num_records > 0){ ?>
                    <div class="isotopeWrapper clearfix isotope lazy-wrapper" data-loader="<?php echo getFromTemplate('common/get_accommodations.php'); ?>" data-mode="click" data-limit="<?php echo $lz_limit; ?>" data-pages="<?php echo $lz_pages; ?>" data-more_caption="<?php echo $texts['LOAD_MORE']; ?>" data-is_isotope="true" data-variables="page_id=<?php echo $page_id; ?>&page_alias=<?php echo $page['alias']; ?>">
                        <?php include(getFromTemplate('common/get_accommodations.php', false)); ?>
                    </div>
                    <?php
                } ?>
            </div>
