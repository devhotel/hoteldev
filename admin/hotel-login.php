...
if($db !== false && isset($_POST['login'])){
    ...
    if(check_token('/'.ADMIN_FOLDER.'/hotel-login.php', 'login', 'post')){
        
        $result_user = $db->query('SELECT * FROM pm_user WHERE login = '.$db->quote($user).' AND pass = '.$db->quote(md5($password)).' AND type= "hotel"  AND checked = 1');
...
<body class="body_hotel_login_new">
    ...
    <div class="loginright">
        ...
        <?php
        if($action == 'reset'){ ?>
        <form id="form" class="form-horizontal" role="form" action="hotel-login.php?action=reset" method="post">
            <?php }
            else {?>
            <form id="form" class="form-horizontal" role="form" action="hotel-login.php" method="post">
            <?php }?>
