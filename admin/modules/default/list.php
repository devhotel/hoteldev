...
if ($db !== false) {
    ...
    if (isset($_REQUEST['search'])) {
        ...
        $q_search = isset($_REQUEST['q_search']) ? htmlentities($_REQUEST['q_search'], ENT_QUOTES, 'UTF-8') : '';
        $q_search = trim(preg_replace('/\s+/', ' ', $q_search));
        $_SESSION['filters'] = serialize($filters);
        $_SESSION['q_search'] = $q_search;
        $offset = 0;
        $_SESSION['offset'] = $offset;
        if (MODULE == 'booking' || MODULE == 'user') {
            if ((isset($_REQUEST['bookFromDt']) && $_REQUEST['bookFromDt'] != '') || (isset($_REQUEST['bookToDt']) && $_REQUEST['bookToDt'] != '') || (isset($_REQUEST['q_search']) && $_REQUEST['q_search'] != '') || $_REQUEST['id_destination'] != ''
            || $_REQUEST['id_hotel'] != '' || $_REQUEST['status'] != '') {
    ...
    // }
    // if ($id_hotel > 0) {
    //     $condition .= 'id_hotel = ' . $id_hotel . ' AND ';
    //     //$condition .= 'id = ' . $id_hotel . ' AND ';
    // }
    $srchFromDt = '';
    $srchToDt = '';
    if (isset($_REQUEST['bookFromDt'])) {
        if($_REQUEST['bookFromDt'] != ''){
            $srchFromDt = $_REQUEST['bookFromDt'];
            $from = strtotime(date($_REQUEST['bookFromDt']. ' 00:00:00')) + 19800;
            $condition .= 'from_date >= ' . $from;
        }
    }
    if (isset($_REQUEST['bookToDt'])) {
        if($_REQUEST['bookToDt'] != ''){
            $srchToDt = $_REQUEST['bookToDt'];
            $end = strtotime(date($_REQUEST['bookToDt'] .' 23:59:59')) + 19800;
            if($condition != ''){
                $condition .= ' AND from_date <= ' . $end ;
            }else{
                $condition .= ' from_date <= ' . $end ;
            }
        }
    }
...
<body>
                        ...
                        <div class="col-md-12 clearfix">
                            ...
                            <div class="pull-left text-right">
                                ...
                                <?php if (MODULE == 'booking') { ?>
                                        <a href="javascript:void(0);" id="btnExport" class="btn btn-success mt15 mb15">
                                            Export To Excel
                                        </a>
                                <?php }
                                $btn = array('booking');
                                ...
                                <div class="panel panel-default">
                                        ...
                                        <div class="row">
                                            <div class="col-md-12 text-left">
                                                <div class="form-inline">
                                                    <?php if (MODULE == 'booking') { ?>
                                                    <div class="<?=(MODULE == 'booking' ? 'form-group-booking' : 'form-group')?>">
                                                        <label for="bookStartDt">From Date </label>
                                                        <input type="text" class="form-control datepicker" placeholder="From date" name="bookFromDt" value="<?=$srchFromDt?>" autocomplete="off">
                                                    </div>
                                                    <div class="<?=(MODULE == 'booking' ? 'form-group-booking' : 'form-group')?>">
                                                        <label for="bookToDt">To Date </label>
                                                        <input type="text" class="form-control datepicker" placeholder="To date" name="bookToDt" value="<?=$srchToDt?>" autocomplete="off">
                                                    </div>
                                                    <?php } ?>
                                                    <?php if (MODULE != 'booking') { ?>
                                                        <div class="form-group">
                                                            <label for="q_search"><?php echo $texts['SEARCH']; ?></label>
                                                            <input type="text" id="list_q_search" name="q_search" value="<?php echo $q_search; ?>" class="form-control input-sm" placeholder="<?php echo $texts['SEARCH']; ?> With Text ..." />
                                                        </div>
                                                    <?php } ?>
                                                    <?php displayFilters($filters, $id_destination, $id_hotel); ?>
                                                    <button class="btn btn-default btn-sm" type="submit" name="search" id="search"> GO </button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="panel-body">
                                        <div class="table-responsive">
                                                    ...
                                                    if ($result !== false) {
                                                        ...
                                                        foreach ($result as $i => $row) {
                                                            ...
                                                            <tr id="item_<?php echo $id ?>" class="<?php echo $class; ?>">
                                                                ...
                                                                foreach ($cols as $col) {
                                                                    ...
                                                                    if (MODULE == 'booking') {
                                                                        if ($name == 'firstname') {
                                                                            ...
                                                                            if ($name == 'source') {
                                                                                echo ucwords(strip_tags(html_entity_decode($col->getValue($i))));
                                                                            } else {
                                                                ...
                                                                if (MODULE == 'booking') {
                                                                    $status = $row['status'];
                                                                    if ($row['payment_option'] == "arrival" || $row['payment_option'] == "2checkout" || $row['payment_option'] == "") {
                                                                        $payments = "<span style='color:#0C0;'>Pay at Hotel</span>";
                                                                    } else {
                                                                        $payments = "<span style='color:#0C0;'>Paid Online</span>";
                                                                    }
                                                                    ?>
                                                                    <td class="text-center">
                                                                  ...
                                                                  <div class="dropdown-menu" aria-labelledby="dropdownMenuButton<?php echo $id; ?>"> -->
                                                                    ...
                                                                    if (in_array('edit', $permissions) || in_array('all', $permissions) || in_array('view', $permissions)) {
                                                                        ...
                                                                        if (MODULE == 'booking') { ?>
                                                                            ...
                                                                            <?php } ?>
                                                                            <?php $today = time();
                                                                            $query_items = 'SELECT from_date, to_date, checked_in, checked_out, status FROM pm_booking WHERE id=' . $id;
                                            ...
                                            <?php if (MODULE == 'booking' || MODULE == 'user') {
                                                // if (($total % $limit) > 0) {
                                                //     $totalPage = strstr(($total / $limit), '.', '-1') + 1;
                                                // } else {
    ...
    $(document).ready(function(){
        $("#btnExport").click(function() {
            //let table = document.getElementsById("listing_base");
            let table = $("#listing_base");
            TableToExcel.convert(table[0], { // html code may contain multiple tables so here we are refering to 1st table tag
            name: `booking.xlsx`, // fileName you could use any name
            sheet: {
