...
function getBookedRooms($from_time, $to_time, $room_id = 0)
{
    ...
    $query_book = '
        ...
        WHERE
			...
			if(!empty($room_id)) $query_book .= '
			AND r.id = '.$room_id;
			if(isset($_SESSION['bt']['sessid'])) $query_book .= '
			AND sessid != '.$db->quote($_SESSION['bt']['sessid']);
			$query_book .= '
...
function getRoomsResult($from_time, $to_time, $num_adults, $num_children, $strict = false, $room_id = 0)
{
    ...
    if($result_rate !== false){
        foreach($result_rate as $i => $row){
            ...
            $num_people =  intval($num_adults)+ intval($num_children);
            ...
            if( intval($num_adults)+ intval($num_children) > $max_people){
                $error = true;
            ...
            if(intval($num_adults)+ intval($num_children) < $min_people){
                $error = true;
            ...
            if(intval($num_adults) > $max_adults){
                $error = true;
                $res_room[$id_room]['notice'] = $texts['MAX_ADULTS'].' : '.$max_adults;
                if(!$strict)
                    $num_adults = $max_adults;
            }
            if(intval($num_children) > $max_children){
                $error = true;
                    ...
                    if(!isset($res_room[$id_room]['total_nights']) || $res_room[$id_room]['total_nights']+$nnights <= $num_nights){
                        ...
                        if($result_tax->execute() !== false){
                            foreach($result_tax as $tax){
                                ...
                                $res_room[$id_room]['taxes']['tax_id'] = $tax['id'];
...
function getAdmRoomsResult($from_time, $to_time, $num_adults, $children, $strict = false, $room_id = 0)
{
    ...
    $query_rate = '
        ...
        WHERE
            ...
            AND ro.checked = 1
            AND stock > 0
            AND max_adults >= '.$nu_adults.'
            AND max_children >= '.$nu_clields.'
            AND (end_lock IS NULL OR end_lock < '.$from_time.' OR
                start_lock IS NULL OR start_lock > '.$to_time.')
            AND start_date <= '.$from_time.'
            AND end_date >= '.$to_time;
