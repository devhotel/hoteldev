...
if(isset($db) && $db !== false){
    ...
    && isset($_POST['to_time']) && is_numeric($_POST['to_time'])){
        ...
        $res_hotel = getRoomsResult($from_time, $to_time, $num_adults, $num_children, true, $room_id);
         //echo '<pre>';
         //print_r($res_hotel);
        //echo $res_hotel[$hotel_id][$room_id];
         //var_dump($res_hotel[$room_id]['taxes']);
        if(!isset($res_hotel[$room_id]) || (isset($res_hotel[$room_id]) && isset($res_hotel[$room_id]['error']) && !isset($res_hotel[$room_id]['notice']))){
            $response['html'] .= '<span class="label label-danger"><i class="fa fa-exclamation-triangle"></i></small></span>
            <input type="hidden" name="unavailable_rooms[]" value="'.$room_id.'">';
            $available = false;
        }elseif(isset($res_hotel[$room_id]) && isset($res_hotel[$room_id]['error']) && isset($res_hotel[$room_id]['notice']))
            $response['html'] .= '<span class="label label-danger"><i class="fa fa-exclamation-triangle"></i> <small>'.$res_hotel[$room_id]['notice'].'</small></span>';
        elseif(isset($res_hotel[$room_id]) && !isset($res_hotel[$room_id]['error'])){
            $amount = $res_hotel[$room_id]['amount']+$res_hotel[$room_id]['fixed_sup'];
            $full_price = $res_hotel[$room_id]['full_price']+$res_hotel[$room_id]['fixed_sup'];
            ...
            <input type="hidden" name="duty_free['.$room_id.']['.$index.']" value="'.number_format($res_hotel[$room_id]['duty_free']+$res_hotel[$room_id]['duty_free_sup'], 10, '.', '').'">';
                            
            if(isset($res_hotel[$room_id]['taxes'])){
                foreach($res_hotel[$room_id]['taxes'] as $tax_id => $tax){
                    //var_dump(@$tax["amount"]);
            ...
            $response['html'] .= ' <i class="fa fa-check-circle text-success"></i> <a href="#btn-book_'.$hotel_id.'" class=""></a>';
            $available = true;
        }
    }
