...
if(!is_numeric($_SESSION['num_adults'])) $field_notice['num_adults'] = $texts2['REQUIRED_FIELD'];
if(!is_numeric($_SESSION['num_children'])) $field_notice['num_children'] = $texts2['REQUIRED_FIELD'];

if($_SESSION['from_date'] == '') $field_notice['dates'] = $texts2['REQUIRED_FIELD'];
else{
    $time = explode('/', $_SESSION['from_date']);
    if(count($time) == 3) $time = gm_strtotime($time[2].'-'.$time[1].'-'.$time[0].' 00:00:00');
    if(!is_numeric($time)) $field_notice['dates'] = $texts2['REQUIRED_FIELD'];
    else $from_time = $time;
}
if($_SESSION['to_date'] == '') $field_notice['dates'] = $texts2['REQUIRED_FIELD'];
else{
...
if(count($field_notice) == 0){
    ...
    if($num_nights <= 0) $msg_error .= $texts2['NO_AVAILABILITY'];
    else{
        ...
        if(empty($res_hotel)) $msg_error .= $texts2['NO_AVAILABILITY'];
...
if($result_hotel !== false){
    ...
    if(!empty($visible_hotels)){
        ...
        $subtitle = str_replace('{min_price}', formatPrice(min($visible_hotels)*CURRENCY_RATE), $texts2['BEST_RATES_SUBTITLE']);
...
if($result_hotel !== false && $db->last_row_count() == 0){
    $msg_error .= $texts2['NO_HOTEL_FOUND'];
    if($destination_name != '') $msg_error .= ' '.$texts2['FOR'].' <b>'.$destination_name.'</b>';
}
...
if(isset($_GET['action'])){
    if($_GET['action'] == 'confirm')
        $msg_success .= $texts2['PAYMENT_SUCCESS_NOTICE'];
    elseif($_GET['action'] == 'cancel')
        $msg_error .= $texts2['PAYMENT_CANCEL_NOTICE'];
}
                        ...
                        <div class="col-md-12 clearfix">
                            ...
                            <div class="pull-left text-right">
                                ...
                                <a href="index.php?view=list">
                                    <button type="button" class="btn btn-default mt15" data-toggle="tooltip" data-placement="bottom" title="<?php echo $texts2['BACK_TO_LIST']; ?>">
                                        <i class="fas fa-fw fa-reply"></i><span class="hidden-sm hidden-xs"> <?php echo $texts2['BACK_TO_LIST']; ?></span>
                                    </button>
                                </a>
                            </div>
                        </div>
            ...
            <div class="row">
                <div class="col-md-<?php echo (isset($destination) || $nb_destinations > 0) ? 9 : 12; ?>">
                    ...
                    <div class="boxed mb20">
                        <?php echo $texts2['BOOKING_NOTICE']; ?>
                    </div>
                                        ...
                                        <div class="col-sm-4 col-md-<?php echo (isset($destination) || $nb_destinations > 0) ? 5 : 6; ?>">
                                            ...
                                            <div class="clearfix mt10">
                                                ...
                                                if($result_hotel_facilities !== false && $db->last_row_count() > 0){
                                                    ...
                                                    <span class="facility-icon">
                                                        <a href="<?php echo DOCBASE.$sys_pages['hotels']['alias'].'/'.text_format($hotel_alias); ?>" title="<?php echo $texts2['READMORE']; ?>" class="tips">...</a>
                                                    </span>
                                                    <?php
                                                } ?>
                                            </div>
                                            ...
                                            <div class="boxed mt10 booking-summary">
                                                ...
                                                <span id="booking-amount_<?php echo $id_hotel; ?>">
													...
													if($num_nights <= 0 || (empty($res_hotel[$id_hotel]) && $room_stock > 0) || (!empty($res_hotel[$id_hotel]) && $room_stock <= 0)){
														...
														<button name="request" class="btn btn-default btn-lg btn-block mt5"><i class="fas fa-fw fa-comment"></i> '.$texts2['MAKE_A_REQUEST'].'</small></button>';
													} ?>
                                                </span>
                                            </div>
                                        </div>
                                                    ...
                                                    foreach($result_room as $row){
                                                        ...
                                                        <div class="row room-result">
                                                            ...
                                                            <div class="col-lg-2 col-md-3 col-sm-3 text-center sep">
                                                                ...
                                                                <div class="mb10 text-muted"><?php echo $texts2['PRICE'].' / '.$type; ?></div>
                                                                <?php echo $texts2['CAPACITY']; ?> : <i class="fas fa-fw fa-male"></i>x<?php echo $max_people; ?>
                                                                
                                                                <?php
                                                                if($room_stock > 0){ ?>
                                                                    <div class="pt10 form-inline">
                                                                        <i class="fas fa-fw fa-tags"></i> <?php echo $texts2['NUM_ROOMS']; ?><br>
                                                                    ...
                                                                    <div class="mt10 btn btn-danger btn-block" disabled="disabled"><?php echo $texts2['NO_AVAILABILITY']; ?></div>
                                                                    <?php
                                                                } ?>
...
<script>
	$(function(){
		$('.room-options').on('change', '[name^="num_children"]', function(){
			...
			var num = $(this).val();
			var html = '<?php echo $texts2['CHILDREN_AGE']; ?>:<br>';
			for(var i = 0; i < num; i++){
				html +=
