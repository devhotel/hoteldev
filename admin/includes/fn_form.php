        ...
        foreach ($fields_table['fields'] as $fieldName => $field) {
            ...
            foreach ($values as $index => $value) {
                ...
                if ($field->isRequired() && $value == '') {
                    $field->setNotice($label . ' is ' . $texts['REQUIRED_FIELD'], $index);
                    $valid = false;
                }
                ...
                switch ($field->getValidation()) {
                    case 'mail':
                        if (!filter_var($value, FILTER_VALIDATE_EMAIL) && $value != '') {
                            $field->setNotice('Please enter a valid email id!!', $index);
                            $valid = false;
                        } elseif ($value == '') {
                            $field->setNotice($label . ' is ' . $texts['REQUIRED_FIELD'], $index);
                            $valid = false;
                        } elseif (!preg_match('/^[\w.-]+@[\w.-]+\.[a-zA-Z]{2,6}$/i', $value)) {
                            $field->setNotice('Please enter a valid email id!!', $index);
                            $valid = false;
                        }
                        break;
                    case 'mobile':
                        if ($value == '' || !preg_match('/^(\+){0,1}(91){0,1}(-|\s){0,1}[0-9]{10}$/i', $value)) {
                            if ($value == '') {
                                $field->setNotice($label . ' is ' . $texts['REQUIRED_FIELD'], $index);
                                $valid = false;
                            } elseif (!preg_match('/^(\+){0,1}(91){0,1}(-|\s){0,1}[0-9]{10}$/i', $value)) {
                                $field->setNotice('Please enter a valid mobile number!!', $index);
                                $valid = false;
                            }
                        }
                        break;
                    case 'phone':
                        if ($value == '' || !preg_match('/^(\+){0,1}(91){0,1}(-|\s){0,1}[0-9]{10}$/i', $value)) {
                            if ($value == '') {
                                ...
                                $field->setNotice('Please enter a valid contact number!!', $index);
                                $valid = false;
                            }
                        }
                        break;
...
function getFieldsFromNode($db, $itemList)
{
    ...
    foreach ($itemList as $item) {
        ...
        if (in_array($_SESSION['admin']['type'], $roles) || in_array('all', $roles)) {
            ...
            if (!empty($itemOptions)) {
                ...
                if ($db !== false && $optionTable != '' && $fieldLabel != '' && $fieldValue != '') {
                    ...
                    if (!in_array($_SESSION['admin']['type'], array('administrator', 'manager', 'editor')) && db_column_exists($db, $optionTable, 'users')) {
                        $query_option_cond .= ($query_option_cond != '') ? ' AND ' : ' WHERE ';
                        $query_option_cond .= 'users REGEXP \'(^|,)' . $_SESSION['admin']['id'] . '(,|$)\'';
                    }
...
function displayField($field, $table, $index, $id_lang)
{
    ...
    switch ($type) {
        case 'id':
            ...
            break;
        case 'text':
            if ($table == 'rate' && ($name == 'price' || $name == 'people')) {
                if (isset($_SESSION['selectedRoomPrice'])) {
                    if ($_SESSION['selectedRoomPrice'] != '' && $name == 'price') {
                        $value =  $_SESSION['selectedRoomPrice'];
                    }
                }
                if (isset($_SESSION['selectedMaxPeople'])) {
                    if ($_SESSION['selectedMaxPeople'] != '' && $name == 'people') {
                        $value =  $_SESSION['selectedMaxPeople'];
                    }
                }
                // if ($_SESSION['selectedRoomPrice'] && $_SESSION['selectedRoomPrice'] != '' && $name == 'price') {
                //     if ($value == '') {
                //         $value =  $_SESSION['selectedRoomPrice'];
                //     }
                // } else if ($_SESSION['selectedMaxPeople'] && $_SESSION['selectedMaxPeople'] != '' && $name == 'people') {
                //     if ($value == '') {
                //         $value =  $_SESSION['selectedMaxPeople'];
                //     }
                // }
            }
            echo '<input type="text"' . $str_active . ' name="' . $inputname . '" id="' . $input_id . '" value="' . $value . '" size="40" rel=' . $inputdescr . ' class="form-control ' . $inputclass . ' ' . (!empty($options) ? ' typeahead' : '') . '">' . "\n";
            ...
            break;
        case 'alias':
            echo '<input type="text"' . $str_active . ' name="' . $inputname . '" id="' . $input_id . '" value="' . $value . '" size="40" rel=' . $inputdescr . ' class="form-control' . (!empty($options) ? ' typeahead' : '') . '">' . "\n";
            if (!empty($options) && $index == 0) {
                echo '
                <script>
                    var dataset_' . $inputdescr . ' = new Bloodhound({
            ...
            break;
        case 'password':
            echo '<input type="password"' . $str_active . ' name="' . $inputname . '" value="" size="40" class="form-control">' . "\n";
            break;
        case 'email':
            echo '<input type="email"' . $str_active . ' name="' . $inputname . '" value="" size="40" class="form-control">' . "\n";
            break;
        case 'tel':
            echo '<input type="tel"' . $str_active . ' name="' . $inputname . '" value="" size="40" class="form-control">' . "\n";
            break;
        case 'textarea':
        ...
        case 'filelist':
            ...
            if ($table == 'rate' && $name == 'id_hotel') {
                if ($_SESSION['selectedHotelId'] && $_SESSION['selectedHotelId'] != '') {
                    foreach ($options as $key => $option) {
                        //echo $key, ' ~ ' . $option . ' ~ ' . $_SESSION['selectedHotelId'] . "\n";
                        if (strval($_SESSION['selectedHotelId']) == strval($key)) {
            ...
            } else if ($table == 'rate' && $name == 'id_room') {
                if (isset($_SESSION['selectedRoomId']) && $_SESSION['selectedRoomId'] != '') {
                    foreach ($options as $key => $option) {
                        if (strval($_SESSION['selectedRoomId']) == strval($key)) {
                            echo '<option value="' . $key . '" selected="selected">' . $option . '</option>' . "\n";
                        }
                    }
                } else {
        ...
        case 'radio':
            foreach ($options as $k => $option) {
                ...
                if ($value == "") {
                    $md = array('coupon');
                    if (in_array(MODULE, $md)) {
                        $checked = ($key == $fkey) ? ' checked="checked"' : '';
                    } else {
                        $checked = '';
                    }
                } else {
            ...
            break;
        case 'hidden':
            echo '<input type="hidden"' . $str_active . ' name="' . $inputname . '" value="' . $value . '" size="40" class="form-control">' . "\n";
            break;
        case 'date':
        case 'datetime':
