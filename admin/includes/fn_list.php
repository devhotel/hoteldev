...
function getCols()
{
    ...
    foreach ($itemList as $item) {
        ...
        if ($fieldValue != '' && $table != '') $key = $table . '.' . $fieldValue;
        else $key = $name;
 ...
 *
 */
function displayFilters($filters, $id_destination = '', $id_hotel = '')
{
    foreach ($filters as $filter) {
        ...
        echo '<div class="'.(MODULE == 'booking' ? 'form-group-booking' : 'form-group').'">';
        echo '<label for="' . $name . '">' . $label . '</label>' . "\n";
        echo '<select name="' . $name . '" id="' . $name . '" class="form-control input-sm"' . $str_filter . '>' . "\n";
        echo '<option value="">All</option>' . "\n";
        foreach ($options as $option) {
            $key = key($options);
            if ($label == 'Destination' && $id_destination > 0) {
                $selected = ($id_destination == $key) ? ' selected="selected"' : '';
            } else if ($label == 'Hotel' && $id_hotel > 0) {
                $selected = ($id_hotel == $key) ? ' selected="selected"' : '';
            } else {
