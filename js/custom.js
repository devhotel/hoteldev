...
$(document).ready(function(){
   var nt= localStorage.getItem("offernight");
    ...
    $(window).scroll(function(){
        ...
        if(scrolltop >= 200) $('a#toTop').css({bottom: '30px', 'z-index': '9999'});
        ...
        $.fn.clear = function(){
            $(this)
                .find('input').not('.noreset')
                    ...
                    .filter(':checkbox').prop('checked', false).removeAttr('checked').end()
                .end()
   ...
   if($('#pfrom_picker').length && $('#pto_picker').length){
            
                $('#pfrom_picker').datepicker({
                    dateFormat: 'dd/mm/yy',
                    minDate: 0,
                    onClose: function(selectedDate){
                        var night = parseInt(nt);
                        var a = selectedDate.split('/');
                        var d = new Date(a[2]+'/'+a[1]+'/'+a[0]);
                        var t = new Date(d.getTime()+(86400*night));
                        var date = (t.getDate()+night)+'/'+t.getMonth()+'/'+t.getFullYear();
                        //$('#pto_picker').datepicker('setDate', date);
                        //$('#pto_picker').datepicker('option', 'minDate', date);
                    }
                });
                $('#pto_picker').datepicker({
                    dateFormat: 'dd/mm/yy',
                });
                $('#pfrom_picker').change(function() {
                  var date2 = $('#pfrom_picker').datepicker('getDate', '+1d'); 
                  var n = parseInt(night);
                  date2.setDate(date2.getDate() + n); 
                  $('#pto_picker').datepicker('setDate', date2);
                  $( "#pto_picker" ).datepicker( "option", "disabled", true );
...
/*    let _from_picker = $("#from_picker").val();
    let _a = _from_picker.split('/');
    let _d = new Date(a[2]+'/'+a[1]+'/'+a[0]);
    let _t = new Date(d.getTime()+86400000);
    let _new_from_picker = t.getDate()+'/'+(t.getMonth()+1)+'/'+t.getFullYear();*/
...
$('#from_picker,#to_picker').keydown(function(e) {
    e.preventDefault();
});
