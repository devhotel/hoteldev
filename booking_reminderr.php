	...
	if ($result_booking !== false) {
		...
		foreach ($rowBooking as $key => $lb) {
			...
			if ($fromdatechk) {
				...
				//SMS Notification
				$message = "Hi " . $recipient_name . ", This is a Reminder. Your booking (#" . $lb['id'] . ") at Met HMS for check-in date is: " . gmstrftime(DATE_FORMAT, $lb['from_date']);
				...
				$from_email = 'reservation@fitser.com';
				$from_name = 'Met HMS';
				$content = '<tr><td style="padding: 20px 20px 0 20px;background: #fff;"><p style="margin: 20px 0 20px;">This is a Reminder.</p><p style="margin: 20px 0 20px;">Your booking (#' . $lb['id'] . ') at Met HMS for check-in date is: ' . gmstrftime(DATE_FORMAT, $lb['from_date']) . '</p>
								<p style="margin: 0 0 20px;">We hope to host you.</p>
								<p style="margin: 0 0 20px;">Cheers <br>Team Met HMS </p></td></tr>';
